...
import { errorStatusEnumMap, ErrorStatus, LCOError } from "../utils/lcoError";

const handleDBCastError = (err: CastError) => {
    const message = `Invalid ${err.path} ${err.value}`;
    return new LCOError(message, 400);
};
...
const handleJWTError = () => new LCOError("Invalid token. Please log in again!", 401);

const handleJWTExpiredError = () =>
    new LCOError("Your token has expired! Please log in again.", 401);
